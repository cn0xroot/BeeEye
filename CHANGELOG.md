...
- **Device-tier connection logging now works on the AF_PACKET path, not only eBPF** (F5): locks/cameras (`DeviceCategory.Sensitivity() == 3`) get every new connection logged as its own event the moment the flow opens, instead of only appearing in the next 5-second aggregated flush — the same treatment the eBPF kernel path already gave them, now also live for the agent's AF_PACKET fallback and for `BeeEye-gui`.
- **Passive device fingerprinting now reaches `identity.Identify`** (F1): DHCP option 55/60, HTTP User-Agent, and SSDP's `Server:` header were already parsed by the dissector but never threaded past the packet-detail tree. `internal/identity` now accepts a `Fingerprint` (vendor-class/user-agent/SSDP hints refine the category, most-specific-first) and DHCP's own `chaddr` is used to attribute a fingerprint to the right device even before it has an IP.
- **DNS tunneling detection** (F33's other gap — NXDOMAIN/DGA detection already existed): `model.DNSRecord` gained a `QType` field (also threaded through from the dissector), and `internal/detect.DNSTunnel` flags a device whose DNS traffic is a burst of TXT/NULL queries, unusually long query names, and heavy concentration under one apex domain — the shape a working covert channel has, which a DGA/NXDOMAIN heuristic alone cannot see.
- **World map: click a destination to pin its details** — hovering already showed a tooltip, but it vanished the moment the pointer moved, and had no click interaction at all. A click now pins the panel open (click empty space to unpin), and it gained a coordinates row and, when an ASN GeoIP database is loaded, the destination's network operator (e.g. "China Telecom (AS4134)") — parsed by `geoip.Lookup` for a while but never carried into `GET /api/views/geopairs`.
- **Traffic Field now has rows for SIP/SCTP/GTP/SIM** — those dissectors shipped a release ago, but the field's fixed 8-channel classifier (`internal/gui.RenderChannels`) never grew to match, so that traffic silently fell into "other". Extended to 12 channels with new CVD-checked colours, mirrored across all four analyzer themes (dark/light/midnight-neon/matrix) and the CSS-side `protocolSlot` classifier the packet list uses, so a packet's row colour and its field row still agree.
- **Traffic Trend: GPU glow is back, as ambient decoration behind the legible SVG chart** — the SVG rewrite that added real axis/gridlines/numbers is unchanged and still what answers "how many bytes", but the still-CUDA-capable `GET /api/render/traffic.png` frame (orphaned since that rewrite) is now layered behind it at low opacity with additive blending, plus a GPU/CPU backend badge next to the legend (same honesty policy as the analyzer's own render badge — never claim GPU rendering that is not actually happening).
- **Traffic Field: protocol share for an imported capture** — a finished offline import is a fixed, complete thing; "what does this file actually consist of" is more useful than watching a live-style field that will never draw another packet. `GET /api/render/totals` (new) exposes a non-decaying bytes-per-channel running total (`Session.channelBytes`, separate from the field's own 82-second-wide rolling history, which would otherwise scroll an already-finished import's data away within about a minute). The channel legend shows each row's share as a percentage and a proportional fill, without reordering the list — it still lines up row-for-row with the field image itself.

- **Traffic Trend: smoothed curve, gradient fills, and correct KB/MB/s formatting**: the line/area paths now use a Catmull-Rom-to-Bezier spline instead of straight segments between samples, area fills fade toward the baseline instead of a flat block, and the line has a subtle colour-matched glow. Both the legend's live rate and the Y-axis labels switched from `formatBytes` (which had no rate-appropriate floor) to `formatRate`, so a near-idle link reads as "0.2 KB/s" instead of a fraction-of-a-byte number like "187.8095245361328 B/s".
- **The capture-report view (program.md's [Pcap-Analyzer](https://github.com/HatBoy/Pcap-Analyzer) acknowledgment) moved from the overview into the analyzer**, opened the same way a live capture is (Toolbar's "Open file"), as a new "Report" tab next to the packet list — summary/protocols/talkers/conversations/sessions/credentials/files/findings/geography, all nine views the overview's old upload-driven tab had. The overview itself dropped that tab in favour of an "Open in Analyzer ↗" link, since the analyzer is where a file is actually inspected now.
- **New GPU-rendered bar chart** (`render.Renderer.RenderBars`, `cuda/BeeEye_render.cu`'s `beeeye_render_bars` kernel + a CPU fallback, both covered by `TestBarsBackendsAgree` — verified pixel-identical on a real RTX 2080 Ti, worst-channel delta 0): the report's protocols/talkers/conversations panels lead with a ranked, glowing bar chart in the same visual language `Render`/`RenderCurve` already use, sharing the exact palette the field and packet list colour their rows with, rather than Pcap-Analyzer's plain tables — `GET /api/report/bars.png?kind=protocols|talkers|conversations`.
- **`./start.sh` now builds and can launch BeeEye-desktop too**: `--desktop` (re)builds the native window shell whenever its Rust sources are stale, same "skip if not stale" rule every other build step already follows, and launches it after both backends are up. Previously the only documented way to build it was `scripts/build-deb.sh`'s own one-off `cargo build --release`.
- **BeeEye-desktop: Analyzer is now the default/first tab, swapped with Overview** — the window opens straight into the packet-level view instead of the overview dashboard.

### Fixed

- **NIC card's live throughput could read as frozen**: `/api/iface/info` had no `Cache-Control` header, leaving a fetch free to reuse a cached response; both the endpoint and the frontend's poll now explicitly disable caching so every 2s tick gets a fresh sample.
- **NIC card's text was cramped/too small**: stat label and value font sizes increased, spacing widened.
- **Traffic Field's content moved/reflowed when the pane was resized horizontally**: the field frame is a fixed-resolution server-rendered raster, and `height: auto` let a resize change the image's *displayed height* along with its width. Height is now pinned to the exact pixel size requested; only width tracks the container, as intended.
- **`formatBytes` could show a raw unrounded float** (e.g. "187.8 B") for any sub-1 KB value fed to it — now rounded, same as every other magnitude it already handled correctly.
- **Traffic Field showed nothing for an offline import**: an imported file replays as fast as it can be read, not paced to real time, so `Session.consume()` often writes every packet into the field's history and flips `status.running` back to `false` again within a single status-poll interval of the import starting — faster than the frontend could ever observe `running` go true. `TrafficField.jsx` gated its frame fetch on `running` alone, so it never requested a frame reflecting the import at all, and then covered whatever was already on screen with an "idle" label. It now also fetches once on every `running: true → false` edge, and only shows the idle label before any frame has ever loaded.
- **World map looked broken ("0 destinations") for traffic that is honestly not plottable**: verified against the live running agent — a GSMTAP/SIM-reader capture (`internal/dissect/gsmtap.go`'s traffic) tunnels over local loopback (`127.0.0.1` on both ends) and has no external IP destination to place on a map, which `GET /api/views/geopairs` correctly omits by design (`c.Internal`/`geo.Local` are filtered). That was indistinguishable from an actual failure. The map now shows an explanatory message instead of a silent blank canvas whenever destinations are zero.
- **Traffic Field's rows were too short to read**: its height was a flat 168px left over from when the field had 8 channels (21px/row); growing to 12 (SIP/SCTP/GTP/SIM) squeezed that to 14px/row without anyone changing the height to match. Now derived from the actual channel count (24px/row, clamped 168–420px) — 12 channels lands on exactly `render.DefaultHeight` (288px), which is not a coincidence.
- **A device could be misclassified wired/wireless on hardware this project's own dev box never happens to exercise**: `isWireless` guessed from the interface name ("starts with `wl`", "contains `wlan`") — correct for the common systemd predictable-naming/legacy `wlanN` cases this was tested against, wrong for a custom udev-renamed interface or an older driver's own convention (`ath0`, `ra0`, ...). Now reads `/sys/class/net/<iface>/phy80211` — the same naming-agnostic signal `iw dev` itself uses — instead of pattern-matching the name at all. `TestIsWirelessAsksTheKernelNotTheName` checks every interface actually present on the machine running the test against that same sysfs path, rather than asserting a name pattern.
- **BeeEye-desktop's Overview tab could stay stuck on "not ready" forever**: the shell only ever auto-started the analyzer backend (BeeEye-gui, :8081) and left BeeEye-agent (:8080, what the Overview tab actually loads) for `start.sh`/`systemd` to have started separately — fine when launched via `./start.sh --desktop`, broken for anyone opening the app on its own (the installed `.deb`'s desktop-launcher entry, most obviously), since nothing had ever asked the agent to run. Inconsistent with the window-close handler, which already stops *both* backends unconditionally regardless of which one it started. Now spawns both symmetrically, the same way it already stopped both — verified end-to-end: launched the desktop binary alone, from a directory with no BeeEye process already running anywhere, and both `:8080` and `:8081` came up on their own.
