...
| F16 | 多网卡可配置采集 | 🟡 | 接口名全部来自 `config/config.yaml`；`captureIface` 依次尝试 config 接口→按角色匹配真实网卡→默认路由网卡→any。**新增按角色（role）的名称无关匹配**：不同机器的内核给网卡起的名字五花八门（`wlan0`/`wlp3s0`/`wlp14s0u2`，`eth0`/`enp2s0`/`eno1`，老驱动的 `ath0`/`ra0`……），过去 config 里配的名字对不上就直接跳过、只能手改 config.yaml（如 v1.3.2 那次专门为 Arch 机器改的 `wlp14s0u2`/`enp12s0`）。现在 `resolveExplicitInterface`（`main.go`）在配置的确切名字不存在时，按该条目的 `role` 字段去本机真实网卡里找：`wifi_ap` 必须是内核认的真无线设备（`livesource.IsWireless`，查 `/sys/class/net/<iface>/phy80211`，跟名字无关，`iw dev` 用的是同一个信号），`wan_uplink` 必须不是；配置的确切名字若存在则始终优先，不会被角色匹配反而覆盖掉运维已经手动配好的设置。**实测**：`main_test.go` 用真实 dummy 网卡验证 exact-name 优先与 `wan_uplink` 角色回退，`wifi_ap` 角色回退用本机真实无线网卡验证（无线网卡环境下才跑，没有则跳过） |
...
| F14 TLS 明文捕获 | 🟡 **阶段一已实现且默认开启**：分析器启动即挂 uprobe 解密网关本机 HTTPS（`internal/gui/decrypt.go`），不需要额外操作。**加密库支持改为声明式规则表**（`internal/tlspeek/rules.go`）：一条 `LibraryRule{Name, SONAME正则, WriteSym, ReadSym}` 覆盖一个库家族，加新库=加一行规则，SONAME 用正则匹配跨版本/跨发行版文件名（`libssl.so.3`、`.so.1.1` 等同一条规则命中，`TestMatchRuleCoversVersionedSONAMEs` 覆盖）；已收录 **OpenSSL** 和 **GnuTLS**（curl/wget 走前者，部分工具走后者），实机同时挂载两族共 **116 个进程**（44 OpenSSL + 72 GnuTLS）。**新增加密库检测**（`internal/tlspeek/detect.go` + `BeeEye-tlspeek -detect` + `GET /api/decrypt/libs`）：用 ELF 动态符号表判定某条库是否真的可挂载（区分"库存在"与"符号未被 strip 因而可挂"），并**从库文件内嵌的版本横幅字符串探测确切版本号**（如 `OpenSSL 3.0.13`、`GnuTLS 3.8.3`），实机同时探测到系统 `OpenSSL 3.0.13` 与 anaconda 环境 `OpenSSL 3.0.16` 两个不同版本并行工作，`TestReadVersionStringOnRealLibraries` 在真实库文件上验证解析不被库内其它含数字的字符串污染（曾误匹配 GnuTLS 错误信息表里的 "GnuTLS error: %s"，已修正为要求版本号后紧跟数字）。**两条路径均已实现**：(A) `BeeEye-tlspeek`/分析器内置 uprobe 解密动态链接 OpenSSL/GnuTLS 进程；(B) `scripts/tls-decrypt.sh` 用 SSLKEYLOGFILE 解密 **Chrome/AdsPower 等 Chromium/Electron**（静态链接 strip 过的 BoringSSL，路径 A 挂不上）。**边界**：路径 A 只限本机、且需符号未 strip；路径 B 需由脚本启动目标。**pcapng+DSB 导出已实现（路径 B）**：`internal/pcapfile/ngwriter.go` 新增 `NgWriter`（`NgReader` 的写入端对称实现），`WriteSecrets` 写标准 Decryption Secrets Block（block type `0x0000000A`，TLS keylog 的 secrets_type `0x544c534b` 取自 Wireshark `wiretap/secrets-types.h`，pcapng 规范本身不定义具体 secrets_type）；`cmd/BeeEye-pcapmerge`（`make pcapmerge`）把一个 `.pcap` + 一份 SSLKEYLOGFILE 合并成一个 `.pcapng`，`scripts/tls-decrypt.sh` 的 `capture`/`decrypt` 收尾自动调用。**范围仅限路径 B**：DSB 装的是密钥，路径 A（uprobe）直接读明文缓冲区、从不产出密钥材料，给它编一行假 keylog 是伪造信号，不做。**实测**（非模拟）：真实 `curl https://example.com`（绕开系统代理直连出口网卡）+ 真实 `tcpdump`（23 个包）+ 真实 `SSLKEYLOGFILE`（5 行 TLS 1.3 密钥）合并后，`tshark -r merged.pcapng` **不指定任何外部 keylog**、仅凭内嵌 DSB 还原出 `GET example.com /` 的真实解密 HTTP/2 请求头；对照组换成一行垃圾 keylog 重新合并，同样查询返回 0 条，排除巧合。详见 [TLS-DECRYPT.md](TLS-DECRYPT.md)。**路径 A 自己的 masterkey 提取（TLS 1.2，OpenSSL 3.0.x）已实现**：`internal/tlspeek/masterkey.go` + `bpf/BeeEye_tls.bpf.c` 的 `try_extract_master_secret`——新增 `ssl_offsets`（用户态按检测到的 OpenSSL 版本写入每 pid 的 `struct ssl_st`/`struct ssl_session_st` 字段偏移量）与 `keylog_seen`（LRU，按 session 指针去重，一条会话只报一次）两张 BPF map，在既有的 `SSL_write` 探针里顺带一次 `bpf_probe_read_user` 读出 `client_random`（固定 32 字节）与 TLS1.2 的 `master_key`（`master_key_length` 必须精确等于 48 才当作有效密钥，非 48 一律跳过，绝不猜长度）。偏移量**不是抄来的**：克隆 OpenSSL 官方 `openssl-3.0.13`/`openssl-3.0.16` 两个 tag（对应本机系统库与 conda 环境库实测到的两个真实版本)，`./Configure linux-x86_64 no-shared` 生成头文件后，用一个 `offsetof()` 探针程序在 `ssl/ssl_local.h` 上现算出 `session`(2328)/`s3.client_random`(352)/`master_key_length`(8)/`master_key`(80) 四个真实字节偏移，两个版本结果完全一致（3.0 分支同一 minor 版本内布局未变），因此按 `"OpenSSL 3.0"` 这个 minor 前缀建表，而不是每个 patch 版本单独建——这是本会话对 eCapture"每版本一张表"技术路线的真实复现，只做了本机实际有的两个版本,不是猜测更多版本可行。`gui/decrypt.go` 的 `attachAll` 在 `Attach()` 成功后顺带调用 `peeker.AttachMasterKey`，检出的密钥**只留在内存**（`Decryptor.keylog`，never written to disk automatically——同 `internal/mitm` 的"最敏感数据只进内存"惯例)，通过 `GET /api/plaintext/keylog` 按需取出（NSS keylog 纯文本，可直接喂给 `BeeEye-pcapmerge -keys`）。**实测**（非模拟，全部真实数据）：真实 `curl --tlsv1.2 --tls-max 1.2 https://example.com`（未用 SSLKEYLOGFILE，纯 uprobe 提取）+ 真实 `tcpdump` 抓包，合并出的 `.pcapng` 里嵌的正是这条 uprobe 提取出的密钥，`tshark` 的 `tls.debug_file` 诊断日志显示 `matched client_random`，`follow,tls,ascii` 完整还原出 HTTP/2 响应体 `<h1>Example Domain</h1>...`（附带解密出的原始页面全文）——路径 A 从此也能产出真正的 DSB，不再是路径 B 独有。**缺口（收窄后）**：GnuTLS/NSS 目前只有明文直读、没有 keylog 提取；Go crypto/tls（需 CO-RE，与 OpenSSL/GnuTLS 走不同架构）；OpenSSL 的 TLS 1.3（`master_key` 字段在 1.3 下语义不同，且各派生密钥长度取决于协商的摘要算法，需要额外的 cipher→哈希长度表，本轮未做，1.3 连接下路径 A 仍只有明文，没有密钥导出)；OpenSSL 非 3.0 分支（1.1.1/3.1+，本机没有可实测的版本，不编造偏移量）；分析器 UI 里的实时明文面板（后端 `/api/plaintext` 已就绪，前端 `Plaintext.jsx` 已接入包详情按 pid 展示，但流式关联仍偏弱，本轮未动——见下方 F14 实测记录）未做完整。参考 [eCapture](https://github.com/gojue/ecapture) 源码（`kern/`、`internal/probe`、`internal/output`）梳理出的具体技术路线见下方"待借鉴细节"，其中"①版本探测+偏移表"一项**两步都已落地**：版本探测（见上）+ OpenSSL 3.0.x 分支的真实偏移表（见上方 masterkey 提取记录）；GnuTLS 偏移表、及 OpenSSL 3.0 之外的其它分支仍未做 |
