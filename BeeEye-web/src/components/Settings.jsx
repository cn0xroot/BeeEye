...
import { THEMES, FONTS, SIZES } from '../theme'

// Settings is the appearance menu: theme, font family and UI scale, each
// applied live. Kept in a popover off a gear button so the header stays clean.
export default function Settings({ theme, onTheme, font, onFont, size, onSize }) {
            ...
            <div className="theme-grid">
              {THEMES.map((th) => (
                ...
                >
                  ...
                  <span className="sw-b" />
                  <span className="sw-c" />
                  <span className="sw-name">{t(`settings.themes.${th}`)}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="settings-group">
            ...
            <div className="seg">
              {Object.keys(FONTS).map((f) => (
                <button key={f} className={font === f ? 'active' : ''} onClick={() => onFont(f)}
          ...
          <div className="settings-group">
            ...
            <div className="seg">
              {Object.keys(SIZES).map((z) => (
                <button key={z} className={size === z ? 'active' : ''} onClick={() => onSize(z)}>
                  {t(`settings.sizes.${z}`)}
                </button>
              ))}
            </div>
