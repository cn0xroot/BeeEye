...
**No capture means an honest empty state, never a fabricated one (F43 — the simulated fallback was removed entirely on 2026-08-21)**: the simulated scenarios (the agent's fixed ten-device `capture.GenerateSimulated`, the analyzer's continuous synthetic-packet `live.OpenSimulated`) are deleted from the codebase, along with the `-simulate` flag and `simulate_seed` config key. With no raw-capture permission, or with neither eBPF nor AF_PACKET available, `live.Open`/`capsource.Open` return an error outright — the agent keeps running with no capture pipeline, and the analyzer's Start call fails immediately — so both the overview and the analyzer show an honest "no data / unavailable" state, with no code path left that can pass fake traffic off as real. The overview UI header carries a live badge (`source-badge`) naming which of `ebpf`/`af_packet`/`unavailable` is actually running.

**Interface selection (F16)**: `captureIface` tries, in order, a configured interface that exists, then the default-route NIC, then `any` — so the shipped `wlan0`/`eth0` that this host lacks resolve to the real NIC instead of silently reporting no data for a NIC that was never actually there.

**`internal/ebpf` today (updated 2026-08-19)**: the CO-RE TC program now supports a "raw-frame mode" — a new `EVT_RAW_FRAME` event kind plus a `CFG_RAW_FRAME_MODE` switch that, once flipped, mirrors every packet's whole raw frame unconditionally (not just the selective DNS/TLS/etc. header regions). `internal/ebpf.OpenEBPF` wraps that into an ordinary `live.Source`, structurally identical to AF_PACKET's, so it feeds the existing dissect pipeline unchanged. `internal/capsource` implements the two-tier chain (eBPF preferred, AF_PACKET fallback) and returns an error outright when neither is available — no third-tier simulator — and `internal/livesource` (the agent) uses it.
...
|---|---|---|---|
| F1 | Device discovery & identity | 🟡 | `internal/identity` infers from OUI + hostname, now extended with a `Fingerprint` (DHCP option 55/60, HTTP User-Agent, SSDP's `Server:` header — `internal/livesource/pipeline.go`'s `seeFingerprint` wires the whole path from dissector to `Identify`, using DHCP's own `chaddr` to attribute a fingerprint before the device even has an IP). **Gap**: still a small hand-curated table (same scale as the existing 19-entry OUI table), not a full Fingerbank database — coverage is limited |
...
| F5 | Tiered monitoring policy | ✅ | Tiering first shipped in the kernel (locks/cameras report per flow, everything else via aggregated snapshots, `bpf/BeeEye.bpf.c`), now matched on the AF_PACKET path: `internal/livesource/pipeline.go` fires an immediate connection event for high-sensitivity categories (`Sensitivity()==3`) the moment a new flow opens, no longer eBPF-only. `TestTieredDeviceFlowReportsImmediately`/`TestUntieredDeviceFlowOnlyAggregates` cover it |
...
| F19 | Multi-theme colors | ✅ | All 9 themes (system, light paper-beige, dark, midnight-neon, matrix, tech-blue, warm-amber, forest-green, high-contrast) implemented and working. The header sun/moon buttons are a light/dark quick toggle; the adjacent gear-icon menu (`Settings.jsx`) exposes the full set as a swatch grid with bilingual labels — every theme is reachable by clicking, no `localStorage` editing needed |
...
| F43 | Capture-source fallback & labeling | ✅ | 2026-08-21: the simulated fallback (`capture.GenerateSimulated`, `live.OpenSimulated`, `-simulate`, `simulate_seed`) is deleted entirely. `live.Open`/`capsource.Open` return an error outright when no real source can be opened — the agent runs with no pipeline, the analyzer's Start fails immediately; the overview header badge and the analyzer status bar only ever show `ebpf`/`af_packet`/`pcap-file`/`unavailable`, with no code path able to present fake data as real |
...
| F33 DNS anomaly detection | ✅ High-rate NXDOMAIN (suspected DGA) implemented; DNS tunneling detection now implemented too — `model.DNSRecord` gained `QType`, `internal/detect.DNSTunnel` flags a TXT/NULL burst, unusually long query names, and heavy concentration under one apex domain. `TestDNSTunnelFlagsTXTBurstToOneDomain`/`TestDNSTunnelIgnoresOrdinaryBrowsing` cover it |
